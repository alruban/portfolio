...
export default class Projects extends React.Component {
  ...
  handleViewport() {
    ...
    const determineSize = (
      mediaStatus: MediaQueryList | MediaQueryListEvent
    ) => {
  ...
  closeEvents() {
    this.profileElements.profileResets.forEach(
      (reset: HTMLElement, index: number) => {
  ...
  randomiseProjectPlacement() {
    const Bounds = (el?: HTMLElement | null, padding = 20) => {
      const box = el?.getBoundingClientRect() ?? {
        left: 0,
        top: 0,
        right: this.projectElements.projectList.offsetWidth - 100,
        bottom: this.projectElements.projectList.offsetHeight - 100,
        width: this.projectElements.projectList.offsetWidth - 100,
        height: this.projectElements.projectList.offsetHeight - 100,
      };
    ...
    const placing = [...this.projectElements.projectItems].map((el) =>
      Bounds(el, 5)
    );
    const fitted: {
      l: number;
      ...
      w: number;
      h: number;
      overlaps(bounds: { r: number; l: number; b: number; t: number }): boolean;
  ...
  render() {
    return projects.map((project: JSONSchema4, index: number) => {
      return (
        <li
          ...
          key={index}
        >
          {project.name == "Bear Brick" ? (
            <BearBrick />
          ) : project.name == "Patchworks" ? (
            <Patchworks />
          ) : project.name == "Climpson & Sons" ? (
            <Climpsons />
          ) : project.name == "Freetrain" ? (
            <Freetrain />
          ) : project.name == "Cedar & Hyde" ? (
            <Cedar />
          ) : project.name == "Dropdeck" ? (
            <Dropdeck />
          ) : project.name == "Smiley" ? (
            <Smiley />
          ) : project.name == "Orb" ? (
